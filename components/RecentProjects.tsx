...
const RecentProjects = () => {
  return (
    <div className="py-20 h-auto xl:mt-12" id="projects">
      <h1 className="heading text-silver-100">
        Recent designs built for{' '}
        <span className="text-gold">Asgardians!</span>
      </h1>
      <div className="flex flex-wrap sm:grid-cols-2 items-center justify-center p-4 gap-x-8 gap-y-8 mt-4 md:flex-row md:flex-wrap md:gap-x-24 md:grid-cols-2 md:gap-y-8 xl:w-11/12 xl:mx-auto xl:gap-x-12 xl:gap-y-16 xl:mt-12">
        {projects.map(({ id, title, des, img, iconLists, link, imgClassName }) => (
              ...
              <div className="flex items-center justify-between mt-7 mb-3">
                <div className="flex items-center">
                  {iconLists.map((icon, index) => (
                    <div
                      key={icon}
                      className="border border-white/[0.2] rounded-full bg-shadow lg:w-10 lg:h-10 w-8 h-8 flex items-center justify-center"
                      style={{
                ...
                <div className="flex justify-center items-center">
                  ...
                  <FaLocationArrow className="ms-3" color="#FFCC11" />
                </div>
              </div>
            </PinContainer>
          </div>
