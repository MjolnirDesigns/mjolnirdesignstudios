...
export const hyperspeedPresets = {
  ...
  six: {
    ...
    colors: {
      ...
      background: 0x000000,
      shoulderLines: 0x7f00ff,
      brokenLines: 0x7f00ff,
      leftCars: [0xff0080, 0x00ffff, 0xffff00, 0xff8000, 0x8000ff],
      rightCars: [0x00ffff, 0xff00ff, 0x00ff80, 0xffff40, 0xff4080],
      sticks: 0xffffff,
    },
  }
};

type PresetKey = keyof typeof hyperspeedPresets;

// Removed duplicate declaration of defaultOptions to fix redeclaration error.

// ← All the massive shader/classes code from your original file goes here (unchanged) →
// (I'm omitting it here for brevity — just paste the entire original code below this line)
...
class Road {
  ...
  createPlane(side: number, width: number, isRoad: boolean) {
    ...
    let uniforms: Record<string, { value: number | THREE.Color | string | boolean | THREE.Vector2 | THREE.Vector3 | THREE.Vector4 }> = {
      uTravelLength: { value: options.length },
      uColor: {
...
// At the very end, replace the old export with this enhanced one:
interface HyperspeedProps {
  preset?: PresetKey;
  effectOptions?: Partial<typeof defaultOptions>;
}

const Hyperspeed: FC<HyperspeedProps> = ({ preset = "one", effectOptions = {} }) => {
