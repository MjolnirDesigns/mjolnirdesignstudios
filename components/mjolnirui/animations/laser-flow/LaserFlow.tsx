// components/mjolnirui/backgrounds/laserflow/LaserFlow.tsx
"use client";
...
// YOUR FULL ORIGINAL SHADER — 100% UNTOUCHED
...
export const LaserFlow: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const onMove: PointerEventListener = (ev) => updateMouse(ev.clientX, ev.clientY);
    canvas.addEventListener('pointermove', onMove as PointerEventListener, { passive: true });
    canvas.addEventListener('pointerdown', onMove as PointerEventListener, { passive: true });
    canvas.addEventListener('pointerenter', onMove as PointerEventListener, { passive: true });

    const onLeave: PointerEventListener = () => {
      mouseTarget.set(0, 0);
    };
    canvas.addEventListener('pointerleave', onLeave as PointerEventListener, { passive: true });
    ...
    return () => {
      ...
      canvas.removeEventListener('pointermove', onMove as PointerEventListener);
      canvas.removeEventListener('pointerdown', onMove as PointerEventListener);
      canvas.removeEventListener('pointerenter', onMove as PointerEventListener);
      canvas.removeEventListener('pointerleave', onLeave as PointerEventListener);
