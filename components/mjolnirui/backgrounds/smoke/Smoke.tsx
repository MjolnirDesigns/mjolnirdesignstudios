...
const Smoke: React.FC<SmokeProps> = ({
  ...
  useEffect(() => {
    ...
    const filmPass = new ShaderPass(new THREE.ShaderMaterial(FilmGrainShader));
  ...
  return (
    <div ref={containerRef} className={`smoke ${className ?? ""}`} style={mergedStyle} />
  );
};
