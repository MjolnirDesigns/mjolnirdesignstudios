...
`;
    ...
    class ShaderPass {
      ...
      constructor(props: unknown) {
        this.props = props || {};
        this.uniforms = (this.props as { material?: { uniforms?: Uniforms } }).material?.uniforms;
      }
      init() {
        this.scene = new THREE.Scene();
        this.camera = new THREE.Camera();
        if (this.uniforms) {
          this.material = new THREE.RawShaderMaterial((this.props as { material: THREE.ShaderMaterialParameters }).material);
      ...
      update() {
        ...
        Common.renderer.setRenderTarget((this.props as { output?: THREE.WebGLRenderTarget | null }).output || null);
    ...
    class ExternalForce extends ShaderPass {
      ...
      // Overload init to match base type signature
      init(): void;
      init(simProps: ExternalForceSimProps): void;
      init(simProps?: ExternalForceSimProps): void {
        super.init();
        if (!simProps) return;
    ...
    class Viscous extends ShaderPass {
      ...
      update(...args: unknown[]) {
        ...
        for (let i = 0; i < iter; i++) {
          const props = this.props as { output0: THREE.WebGLRenderTarget; output1: THREE.WebGLRenderTarget; output?: THREE.WebGLRenderTarget };
          if (i % 2 === 0) {
            fbo_in = props.output0;
            fbo_out = props.output1;
          } else {
            fbo_in = props.output1;
            fbo_out = props.output0;
          }
          this.uniforms.velocity_new.value = (fbo_in as THREE.WebGLRenderTarget).texture;
          props.output = fbo_out as THREE.WebGLRenderTarget;
          if (typeof dt === 'number') this.uniforms.dt.value = dt;
          super.update();
        }
        return fbo_out;
    ...
    class Divergence extends ShaderPass {
      ...
      update(options?: { vel?: THREE.WebGLRenderTarget }) {
        ...
        if (
          ...
          this.uniforms.velocity.value = (vel as THREE.WebGLRenderTarget).texture;
        }
        super.update();
      }
    }
    ...
    class Poisson extends ShaderPass {
      lastOutput?: THREE.WebGLRenderTarget;
      ...
      update(options?: { iterations?: number }): void {
        ...
        // Store the last p_out in a class property for later use.
        this.lastOutput = p_out;
      }
    }
    ...
    class Pressure extends ShaderPass {
      ...
      update(...args: unknown[]) {
        ...
        if (
          ...
          this.uniforms.velocity.value = (vel as THREE.WebGLRenderTarget).texture;
          this.uniforms.pressure.value = (pressure as THREE.WebGLRenderTarget).texture;
        }
        super.update();
      }
    }
    ...
    class Simulation {
      ...
      createShaderPass() {
        this.advection = new Advection({
          ...
          dt: this.options.dt,
          src: this.fbos.vel_0 ? { texture: this.fbos.vel_0.texture } : { texture: new THREE.Texture() },
          dst: this.fbos.vel_1!
        });
        ...
        this.viscous = new Viscous({
          ...
          src: { texture: this.fbos.vel_1 ? this.fbos.vel_1.texture : new THREE.Texture() },
        ...
        this.divergence = new Divergence({
          cellScale: this.cellScale,
          boundarySpace: this.boundarySpace,
          src: { texture: this.fbos.vel_viscous0 ? this.fbos.vel_viscous0.texture : new THREE.Texture() },
          dst: this.fbos.div!,
          dt: this.options.dt
        });
        this.poisson = new Poisson({
          ...
          boundarySpace: this.boundarySpace,
          src: this.fbos.div!,
          dst: this.fbos.pressure_1 ?? new THREE.WebGLRenderTarget(this.fboSize.x, this.fboSize.y),
          dst_: this.fbos.pressure_0!
        });
        this.pressure = new Pressure({
          cellScale: this.cellScale,
          boundarySpace: this.boundarySpace,
          src_p: this.fbos.pressure_0 ?? new THREE.WebGLRenderTarget(this.fboSize.x, this.fboSize.y),
          src_v: this.fbos.vel_viscous0 ?? new THREE.WebGLRenderTarget(this.fboSize.x, this.fboSize.y),
          dst: this.fbos.vel_0 ?? new THREE.WebGLRenderTarget(this.fboSize.x, this.fboSize.y),
          dt: this.options.dt
        });
      }
      calcSize() {
    ...
    // Define the WebGLManager class after the interface
    class WebGLManager {
      ...
      constructor(props: WebGLManagerProps) {
        ...
        this.resize = this._resizeImpl.bind(this);
        this._resize = this.resize.bind(this);
        this.start = this._startImpl.bind(this);
        this.pause = this._pauseImpl.bind(this);
        this.dispose = this._disposeImpl.bind(this);
      }
    
      loop(this: WebGLManager): void {
      ...
      // Explicitly declare interface methods
      public resize!: () => void;
      public start!: () => void;
      public pause!: () => void;
      public dispose!: () => void;
      ...
      private _startImpl(): void {
        if (this.running) return;
        this.running = true;
        this._loop();
      }
      private _pauseImpl(): void {
        this.running = false;
        if (rafRef.current) {
